    ...
    setTimeout(() => {
      if (basketRef.current) {
        basketRef.current.measureInWindow((x, y, width, height) => {
          onSetBasketCoords({ x: x + width / 2, y, width, height });
...
function TicketScreen() {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      ...
      headerRight: () => <Basket onSetBasketCoords={handleSetBasketCoords} />,
    });
  }, [navigation]);
  const handleSetBasketCoords = React.useCallback(({ x, y, width, height }) => {
    setBasketCoords({ x, y, width, height });
  }, []);
